...
public class Grammar
{
    ...
    private Map<String, String> renameNt; // single-char non-terminals renamed globally
    ...
    private void readGrammar(String path) throws IOException
    {
        ...
        Set<String> lhsSeen = new LinkedHashSet<>();
        ...
        while ((line = br.readLine()) != null)
        {
            ...
            rawRules.add(new String[] { nt, rhs });
            lhsSeen.add(nt);
        }

        br.close();

        buildRenameMap(lhsSeen);

        for (String[] rule : rawRules)
        {
            String nt = renameToken(rule[0]);
            String rhs = rule[1];
    ...
    private void buildRenameMap(Set<String> lhsSeen)
    {
        Random random = new Random();
        Set<String> used = new LinkedHashSet<>(lhsSeen);

        for (String nt : lhsSeen)
        {
            if (nt.length() == 1)
            {
                String candidate;
                do
                {
                    candidate = nt + random.nextInt(10);
                }
                while (used.contains(candidate));

                renameNt.put(nt, candidate);
                used.add(candidate);
            }
        }
    }

    private String renameToken(String token)
    {
        return renameNt.getOrDefault(token, token);
    }
    ...
    private String makePrime(String base)
    {
